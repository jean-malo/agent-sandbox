...
func TestOwnedSandboxUpdateAffectsClaimReconcile(t *testing.T) {
	now := metav1.Now()
	base := &sandboxv1beta1.Sandbox{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				sandboxv1beta1.SandboxPodNameAnnotation: "test-sandbox",
			},
			Finalizers: []string{"cleanup.example.com"},
			OwnerReferences: []metav1.OwnerReference{
				{
		...
		Spec: sandboxv1beta1.SandboxSpec{
			PodTemplate: sandboxv1beta1.PodTemplate{
				Spec: corev1.PodSpec{
					Containers: []corev1.Container{{Name: "main", Image: "python:3.12"}},
				},
			},
		},
		Status: sandboxv1beta1.SandboxStatus{
	...
	testCases := []struct {
		...
		{
			name: "service fqdn status change does not enqueue",
			edit: func(sandbox *sandboxv1beta1.Sandbox) {
				sandbox.Status.ServiceFQDN = "test-sandbox.default.svc.cluster.local"
			},
			want: false,
		},
		{
			name: "selector status change does not enqueue",
			edit: func(sandbox *sandboxv1beta1.Sandbox) {
				sandbox.Status.LabelSelector = "app=test"
			},
			want: false,
		},
		{
			name: "node name status change does not enqueue",
			edit: func(sandbox *sandboxv1beta1.Sandbox) {
				sandbox.Status.NodeName = "node-1"
			},
			want: false,
		},
		{
			name: "ready condition change enqueues",
			edit: func(sandbox *sandboxv1beta1.Sandbox) {
		...
		{
			name: "finished condition change enqueues",
			edit: func(sandbox *sandboxv1beta1.Sandbox) {
				meta.SetStatusCondition(&sandbox.Status.Conditions, metav1.Condition{
					Type:               string(sandboxv1beta1.SandboxConditionFinished),
					Status:             metav1.ConditionTrue,
		...
		},
		{
			name: "spec change enqueues",
			edit: func(sandbox *sandboxv1beta1.Sandbox) {
				sandbox.Spec.PodTemplate.Spec.Containers[0].Image = "python:3.13"
			},
			want: true,
		},
		{
			name: "label change enqueues",
			edit: func(sandbox *sandboxv1beta1.Sandbox) {
				sandbox.Labels["example.com/label"] = "value"
			},
			want: true,
		},
		{
			name: "owner reference change enqueues",
			edit: func(sandbox *sandboxv1beta1.Sandbox) {
				sandbox.OwnerReferences[0].UID = "other-claim-uid"
			},
			want: true,
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			updated := base.DeepCopy()
			tc.edit(updated)
			require.Equal(t, tc.want, ownedSandboxUpdateAffectsClaimReconcile(base, updated))
		})
	}
}
...
// TestSandboxClaimCompletesAssignedWarmPoolAdoption verifies that if a claim
// already records an assigned warm-pool Sandbox, the controller completes that
// adoption and continues the same reconcile instead of adopting another sandbox.
func TestSandboxClaimCompletesAssignedWarmPoolAdoption(t *testing.T) {
	scheme := newScheme(t)
	...
	if controllerRef == nil {
		t.Fatal("expected adopted sandbox to have a controller reference")
	}
	if controllerRef.Kind != "SandboxClaim" || controllerRef.Name != "test-claim" || controllerRef.UID != "claim-uid-123" {
		t.Errorf("expected adopted sandbox to be controlled by claim, got %#v", controllerRef)
	}
	if _, ok := adopted.Labels[warmPoolSandboxLabel]; ok {
		t.Error("expected adopted sandbox to have warm pool label removed")
	}
...
func TestSandboxClaimAdoptionRebuildsEmptyWarmPoolQueue(t *testing.T) {
	scheme := newScheme(t)
	const (
		namespace    = "default"
		poolName     = "test-pool"
	...
	fakeClient := fake.NewClientBuilder().
		WithScheme(scheme).
		WithObjects(template, claim, warmPool, warmSandbox).
		WithStatusSubresource(claim).
		Build()
	...
	req := reconcile.Request{NamespacedName: types.NamespacedName{Name: claimName, Namespace: namespace}}
	_, err := reconciler.Reconcile(context.Background(), req)
	require.NoError(t, err)

	var adoptedSandbox sandboxv1beta1.Sandbox
	err = fakeClient.Get(context.Background(), types.NamespacedName{Namespace: namespace, Name: sandboxName}, &adoptedSandbox)
	require.NoError(t, err)
	controllerRef := metav1.GetControllerOf(&adoptedSandbox)
	require.NotNil(t, controllerRef)
	...
	var updatedClaim extensionsv1beta1.SandboxClaim
	err = fakeClient.Get(context.Background(), types.NamespacedName{Namespace: namespace, Name: claimName}, &updatedClaim)
	require.NoError(t, err)
	require.Equal(t, sandboxName, updatedClaim.Annotations[extensionsv1beta1.AssignedSandboxNameAnnotation])

	var coldSandbox sandboxv1beta1.Sandbox
	err = fakeClient.Get(context.Background(), types.NamespacedName{Namespace: namespace, Name: claimName}, &coldSandbox)
	require.True(t, k8errors.IsNotFound(err), "expected warm adoption, not cold-start sandbox creation")
}
